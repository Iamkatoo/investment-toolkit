...
class FMPAPI:
    ...
    def get_historical_price(self, symbol, from_date=None, to_date=None):
        ...
        # APIレスポンスの形式を確認（stableエンドポイントとv3 APIでは形式が異なる）
        is_stable_endpoint = isinstance(data, list)
        is_v3_api = isinstance(data, dict) and 'historical' in data
        
        # adj_closeカラムがない場合は、closeの値をコピー
        if 'adj_close' not in df.columns and 'close' in df.columns:
            self.logger.debug(f"{symbol}: adj_closeカラムがないため、closeの値をコピーします")
        ...
        if not df.empty:
            ...
            if symbol.startswith('^'):
                self.logger.debug(f"{symbol}は株価指数です。特別な処理を適用します。")
                
                # volumeに関するデータ修正
                if 'volume' in df.columns and 'change' in df.columns:
                    sample_volume = df['volume'].iloc[0]
                    sample_change = df['change'].iloc[0]
                    
                    # 数値型かどうかをチェック
                    if (isinstance(sample_volume, (int, float)) and 
                        isinstance(sample_change, (int, float))):
                        
                        # volumeが小さく、changeが大きい場合は値が入れ替わっている可能性がある
                        if sample_volume < 1000 and sample_change > 10000:
                            self.logger.warning(f"{symbol}: volumeとchangeの値が入れ替わっている可能性があります。修正します。")
                            temp_volume = df['volume'].copy()
                            df['volume'] = df['change']
                            df['change'] = temp_volume
                            self.logger.info(f"{symbol}: volumeとchangeの値を入れ替えました")
                
                # v3 API形式の場合、特別な処理
                if is_v3_api:
                    # volumeとadj_closeの関係をチェック
                    if 'volume' in df.columns and 'adj_close' in df.columns:
                        sample_volume = df['volume'].iloc[0]
                        sample_adj_close = df['adj_close'].iloc[0]
                        
                        # v3 APIで株価指数のvolumeとadj_closeが逆になっている異常ケース
                        if (isinstance(sample_volume, (int, float)) and 
                            isinstance(sample_adj_close, (int, float)) and
                            sample_adj_close > 100000 and sample_volume < 100):
                            
                            self.logger.warning(f"{symbol}: v3 APIでvolumeとadj_closeの値が入れ替わっています。修正します。")
                            # 値を入れ替え
                            temp_adj_close = df['adj_close'].copy()
                            df['adj_close'] = df['volume']
                            df['volume'] = temp_adj_close
                            self.logger.info(f"{symbol}: volumeとadj_closeの値を入れ替えました")
            
            # 異常なNaN値や型の問題を修正
            for col in ['volume', 'change', 'unadjusted_volume', 'adj_close']:
                if col in df.columns:
                    # NaN値をチェック
                    if df[col].isna().any():
                        self.logger.warning(f"{symbol}: {col}列にNaN値があります")
                        if col in ['volume', 'unadjusted_volume']:
                            # ボリュームのNaNは0に置き換え
                    ...
                    if df[col].dtype == 'object':
                        try:
                            ...
                            self.logger.info(f"{symbol}: {col}列を数値型に変換しました")
                        except Exception as e:
                            self.logger.error(f"{symbol}: {col}列の数値変換中にエラー: {e}")
            
            # vwapの値のチェック
            if 'vwap' in df.columns and 'adj_close' in df.columns:
                # vwapとadj_closeが同じ値になっている場合
                if df['vwap'].equals(df['adj_close']):
                    self.logger.warning(f"{symbol}: vwapとadj_closeの値が同じです")
                    # vwapを(high + low + close) / 3で再計算
                    if all(col in df.columns for col in ['high', 'low', 'close']):
                        df['vwap'] = (df['high'] + df['low'] + df['close']) / 3
                        self.logger.info(f"{symbol}: vwapの値を再計算しました")
        ...
        if not df.empty and 'date' in df.columns:
            ...
            self.logger.info(f"{symbol}の株価データ: {len(df)}件取得（期間: {min_date} ～ {max_date}）")
    ...
    def get_forex(self, symbol, from_date=None, to_date=None):
        ...
        """
        endpoint = f"/historical-price-eod/light"
        params = {
            'symbol': symbol
        }
