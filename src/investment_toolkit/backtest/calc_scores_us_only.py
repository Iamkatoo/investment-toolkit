...
"""
改良版総合スコア計算（US株のみ・フィルタリング統合版）

US市場専用のスコア計算：
- フィルタリング機能統合（全銘柄計算 + 状態記録）
...
class ImprovedScoreCalculatorUS:
    """
    改良版総合スコア計算クラス（US株専用・フィルタリング統合版）
    """
    ...
    def check_quality_growth_filter(self, row: pd.Series) -> bool:
        ...
        if pd.notna(row.get('roic')):
            if row['roic'] < self.quality_filters['min_roic']:
                return True
        
        # Debt/Equity最大値チェック
        if pd.notna(row.get('debt_to_equity')):
            if row['debt_to_equity'] > self.quality_filters['max_debt_equity']:
                return True
        
        # EPS CAGR 3y最低値チェック
        if pd.notna(row.get('eps_cagr_3y')):
            if row['eps_cagr_3y'] < self.quality_filters['min_eps_cagr_3y']:
                return True
        
        # Altman Z代理指標チェック
        if pd.notna(row.get('roe')) and pd.notna(row.get('roic')):
            altman_proxy = (row['roe'] + row['roic']) / 2
    ...
    def check_value_trap_filter(self, row: pd.Series) -> bool:
        ...
        """
        # PER範囲チェック
        if pd.notna(row.get('per')):
            per = row['per']
            if per < self.value_filters['min_per'] or per > self.value_filters['max_per']:
                return True
        
        # PBR最低値チェック
        if pd.notna(row.get('pbr')):
            if row['pbr'] < self.value_filters['min_pbr']:
                return True
    ...
    def load_master_data(self, start_date: str = "2015-01-01", end_date: Optional[str] = None, target_date: Optional[str] = None) -> pd.DataFrame:
        ...
        Returns:
            マスターデータのDataFrame
        """
        logger.info("マスターデータ読み込み開始（US株のみ）...")
        ...
        WHERE vm.per IS NOT NULL 
        AND vm.close > 0
        AND vm.symbol NOT LIKE '%.T'
        {date_condition}
        ...
        try:
            ...
            logger.info(f"マスターデータ読み込み完了（US株のみ）: {len(df):,}行, {df['symbol'].nunique():,}銘柄")
            return df
        except Exception as e:
    ...
    def calculate_daily_scores(self, df: pd.DataFrame, target_date: Optional[str] = None, limit_days: Optional[int] = None) -> pd.DataFrame:
        """
        改良版日次スコア計算のメイン処理（US株専用・全銘柄計算 + フィルタリング状態記録）
        ...
        logger.info(f"改良版US株スコア計算開始（全銘柄対象）: {len(dates)}日分")
        
        # DataFrameの日付列を文字列に統一
        df['date'] = pd.to_datetime(df['date']).dt.strftime('%Y-%m-%d')
        
        for date in tqdm(dates, desc="改良版US株スコア計算中"):
            ...
            logger.info(f"日付 {date_str}: {len(daily_df)}銘柄のデータを処理中（US株・全銘柄対象）")
        ...
        logger.info(f"改良版US株スコア計算完了: {len(results)}件の結果")
    ...
    def save_scores(self, scores_df: pd.DataFrame) -> None:
        ...
        try:
            logger.info(f"US株スコアデータ保存開始: {len(scores_df)}行")
            
            # 古いデータを削除（US株のみ・同じ日付）
            dates = scores_df['date'].unique()
            
            logger.info(f"既存US株データ削除中: {len(dates)}日分")
            
            with self.engine.connect() as conn:
                for date in dates:
                    delete_query = text("""
                    DELETE FROM backtest_results.daily_scores 
                    WHERE date = :date AND symbol NOT LIKE '%.T' AND symbol NOT LIKE '%.L'
                    """)
                    result = conn.execute(delete_query, {"date": date})
                    logger.info(f"日付 {date}: {result.rowcount} 行のUS株データを削除")
                conn.commit()
            ...
            logger.info("改良版US株スコアデータ保存完了（フィルタリング状態含む）")
            
        except Exception as e:
            logger.error(f"US株スコアデータ保存エラー: {e}")
            raise
...
def main():
    ...
    parser = argparse.ArgumentParser(description="改良版総合スコア計算（US株のみ・フィルタリング統合版）")
